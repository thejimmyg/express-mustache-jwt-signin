...
const setupMiddleware = (secret, options) => {
  ...
  return { withUser, signedIn }
}

const setupLogin = (app, secret, credentials, options) => {
  const { dashboardURL = '/user/dashboard', signedOutTemplate='signedOut', signInTemplate='signIn', signedOutTitle='Signed Out', signInTitle='Sign In', jwtCookieName = DEFAULT_COOKIE_NAME, extractTokenFromRequest = extractTokenFromRequestDefault, signOutURL = '/user/signout', signInURL = DEFAULT_SIGNIN_URL, ...rest } = options || {}
  if (Object.keys(rest).length) {
    throw new Error('Unexpected extra options: ' + Object.keys({ rest }).join(', '))
  }
  ...
  const { signedIn, withUser } = setupMiddleware(secret, { signInURL, extractTokenFromRequest, jwtCookieName })
