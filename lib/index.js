...
const signInOptionsFromEnv = () => {
  return {
    ...
    signOutUrl: process.env.SIGN_OUT_URL,
    signInUrl: process.env.SIGN_IN_URL
  }
}

const appOptionsFromEnv = (scriptName) => {
...
const setupMiddleware = async (app, secret, options) => {
  const { overlays, withPjaxPwa=false, signInUrl = '/user/signin', extractTokenFromRequest = extractTokenFromRequestDefault, cookieName = 'jwt', forbiddenTemplate = '403', forbiddenTitle = 'Forbidden', signOutUrl = '/user/signout', ...rest } = options || {}
...
module.exports = { setupLogin, setupMiddleware, makeStaticWithUser, signInOptionsFromEnv, appOptionsFromEnv }
