...
const setupLogin = (app, secret, credentials, options) => {
  const { dashboardURL = '/user/dashboard', jwtCookieName = DEFAULT_COOKIE_NAME, extractTokenFromRequest = extractTokenFromRequestDefault, signOutURL = '/user/signout', signInURL = DEFAULT_SIGNIN_URL, ...rest } = options || {}
  if (Object.keys(rest).length) {
    throw new Error('Unexpected extra options: ' + Object.keys({ rest }).join(', '))
  }
