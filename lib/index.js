...
const SIGN_IN_URL = process.env.SIGN_IN_URL || SCRIPT_NAME + '/signin'
const DASHBOARD_URL = process.env.DASHBOARD_URL || SCRIPT_NAME + '/dashboard'
const ADMIN_URL = process.env.ADMIN_URL || SCRIPT_NAME + '/admin'
const HASH_URL = process.env.HASH_URL || SCRIPT_NAME + '/hash'
...
const SIGNOUT_URL = process.env.SIGN_OUT_URL || SCRIPT_NAME + '/signout'
...
const setupLogin = (app, secret, credentials, options) => {
  const { httpsOnly = HTTPS_ONLY, adminURL= ADMIN_URL, hashURL=HASH_URL, dashboardURL = DASHBOARD_URL, signedOutTemplate = 'signedOut', publicURLPath, signInTemplate = 'signIn', signedOutTitle = 'Signed Out', signInTitle = 'Sign In', jwtCookieName = COOKIE_NAME, extractTokenFromRequest = extractTokenFromRequestDefault, signOutURL = SIGN_OUT_URL, scriptName, signInURL = SIGN_IN_URL, title, forbiddenTemplate = FORBIDDEN_TEMPLATE, forbiddenTitle = FORBIDDEN_TITLE, ...rest } = options || {}
  if (Object.keys(rest).length) {
    throw new Error('Unexpected extra options: ' + Object.keys( rest ).join(', '))
  }
  ...
  app.use((req, res, next) => {
    ...
    res.locals = Object.assign({}, res.locals, { adminURL, hashURL, dashboardURL, signOutURL, signInURL, publicURLPath, scriptName, title})
    next()
  })
