...
const DEFAULT_SIGNIN_URL = scriptName + '/signin'
const DEFAULT_COOKIE_NAME = 'jwt'
const DEFAULT_FORBIDDEN_TEMPLATE = '403'
const DEFAULT_FORBIDDEN_TITLE = 'Forbidden'

const setupMiddleware = (secret, options) => {
  const { signInURL = DEFAULT_SIGNIN_URL, extractTokenFromRequest = extractTokenFromRequestDefault, jwtCookieName = DEFAULT_COOKIE_NAME, forbiddenTemplate = DEFAULT_FORBIDDEN_TEMPLATE, forbiddenTitle = DEFAULT_FORBIDDEN_TITLE, ...rest } = options || {}
...
const setupLogin = (app, secret, credentials, options) => {
  const { httpsOnly = true, dashboardURL = scriptName + '/dashboard', signedOutTemplate = 'signedOut', signInTemplate = 'signIn', signedOutTitle = 'Signed Out', signInTitle = 'Sign In', jwtCookieName = DEFAULT_COOKIE_NAME, extractTokenFromRequest = extractTokenFromRequestDefault, signOutURL = scriptName + '/signout', signInURL = DEFAULT_SIGNIN_URL, forbiddenTemplate = DEFAULT_FORBIDDEN_TEMPLATE, forbiddenTitle = DEFAULT_FORBIDDEN_TITLE, ...rest } = options || {}
  if (Object.keys(rest).length) {
    throw new Error('Unexpected extra options: ' + Object.keys({ rest }).join(', '))
  }
