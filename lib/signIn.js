...
const prepareSignIn = (app, signInOptions) => {
  ...
  if (!signInOptions.dashboardUrl) {
    throw new Error('Using signInOptions, but no signInOptions.dashboardUrl set. Perhaps you need to set the DASHBOARD_URL environment variable?')
  }
  if (!signInOptions.signOutUrl) {
    throw new Error('Using signInOptions, but no signInOptions.signOutUrl set. Perhaps you need to set the SIGN_OUT_URL environment variable?')
  }
